...
Current focus: `The live triage gate has now been loosened slightly so AI_MODE=triage still spends budget on the most valuable rows without suppressing same-product-area multi-source cases too aggressively. code/ai_validation.py now treats near-tied top retrieval chunks from different source documents as triage-worthy even when they map to the same product_area, which admits a few additional synthesis-worthy rows while keeping duplicate-chunk single-document cases gated out. Fresh validation reran AI_MODE=triage sample and manual regression checks cleanly, and a fresh 29-row live triage batch recorded llm_called=15, triage_accepted=15, triage_skipped_low_value=3, vetoed_high_risk_safety=11, Changed rows vs baseline 5, and Flagged rows 0.`
