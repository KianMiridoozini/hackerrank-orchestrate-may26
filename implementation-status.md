...
Current focus: `strengthen-triage.md T3, T4, and T5 are now verified. code/agent.py now renders a richer triage prompt with company_or_domain, deterministic_escalation_kind, per-snippet candidate_product_area metadata, and explicit rule-style instructions about using only local evidence, avoiding URLs/internal reasoning, preserving fallback categories unless evidence clearly supports a change, and respecting hard_safety_veto. code/ai_validation.py now also rejects ambiguous product_area changes when the top retrieved evidence still supports the deterministic fallback area more strongly, and its forbidden-marker validation now blocks additional prompt-marker leakage such as candidate_product_area and fallback_should_escalate_reason. The central ai-trace path now adds top-level ai_mode, decision, rejection_reason, hard_safety_veto, budget_reasons, evidence_support, and accepted categorical_changes, while synthesis and triage mode traces preserve evidence_support before later validation can reject a row. Focused validation confirmed accepted and rejected trace rows carry the required normalized fields and that categorical deltas appear only on accepted changes. Standard and aggressive AI_MODE=triage reruns of evaluate_sample.py and manual_regressions.py remain clean from T4 at 10/10 status, 10/10 request_type, 8/8 product_area, and 9/9 manual regressions.`

Current recommended next action: `Start T6 by rerunning the full off, triage, aggressive, sample, regression, and inspection validation matrix against the strengthened trace schema.`
