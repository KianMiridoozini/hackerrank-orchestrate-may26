...
Current focus: `strengthen-triage.md is now underway as a tracked follow-on sequence. T1 and T2 are both verified. Hard safety categories now expose a dedicated hard-safety-veto helper, AI_MODE=triage may attempt wording-only improvements for those rows, and any attempt to change a hard-veto row away from its deterministic escalated status/request_type/product_area is rejected and falls back cleanly. The new AI_TRIAGE_AGGRESSIVE=1 override now bypasses triage budget gating for every non-invalid row with retrieved evidence while leaving default AI_MODE=triage unchanged. Focused fake-transport validation confirmed both the hard-veto rejection contract and the aggressive budget-bypass contract, and repo-level reruns under aggressive mode stayed clean at 10/10 status, 10/10 request_type, 8/8 product_area, 9/9 manual regressions, Changed rows vs baseline 7, and Flagged rows 0.`

Current recommended next action: `Start T3 by enriching the triage prompt with explicit hard-versus-soft escalation context, cleaner evidence snippets, and stronger wording constraints before tightening acceptance and traceability in T4 and T5.`
...
| T3 | Improve triage prompt | NOT_STARTED | - | - | Expand the prompt with explicit hard/soft escalation context and cleaner evidence snippets. |
