...
Current focus: `strengthen-triage.md T3 and T4 are now verified. code/agent.py now renders a richer triage prompt with company_or_domain, deterministic_escalation_kind, per-snippet candidate_product_area metadata, and explicit rule-style instructions about using only local evidence, avoiding URLs/internal reasoning, preserving fallback categories unless evidence clearly supports a change, and respecting hard_safety_veto. code/ai_validation.py now also rejects ambiguous product_area changes when the top retrieved evidence still supports the deterministic fallback area more strongly, and its forbidden-marker validation now blocks additional prompt-marker leakage such as candidate_product_area and fallback_should_escalate_reason. Focused fake-transport validation confirmed the previously bad privacy-to-conversation_management change is now rejected while a broad general_support fallback can still upgrade to a clearly supported specific area. Standard and aggressive AI_MODE=triage reruns of evaluate_sample.py and manual_regressions.py are clean again at 10/10 status, 10/10 request_type, 8/8 product_area, and 9/9 manual regressions. A fresh aggressive batch wrote 29 rows, inspect_output.py reported Changed rows vs baseline: 2 and Flagged rows: 0, and the latest 29-row trace slice recorded 20 accepted rows, 7 justification-content rejections, 2 invalid skips, and no accepted categorical changes.`

Current recommended next action: `Start T5 by extending ai_trace entries with normalized accepted/rejected/skipped status, evidence_support, hard_safety_veto, and categorical-delta details before the full T6 validation matrix.`
